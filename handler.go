...
// this handler will generate JWT token to authenticate
// username & password that pass through http call.
...
func Refresh(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == http.ErrNoCookie {
			fmt.Println("No token provided when send request")
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
		w.WriteHeader(http.StatusBadRequest)
	...
	if time.Until(claims.ExpiresAt.Time) > 3*time.Minute {
		fmt.Println("token lifetime is more than 3 minutes")
		w.WriteHeader(http.StatusBadRequest)
		return
	}
